...
#[tokio::test]
#[cfg(feature = "callbacks")]
async fn reorg_caught_on_startup() -> anyhow::Result<()> {
    let env = testenv()?;
    ...
    let LightClient {
        ...
        mut receiver,
        node,
    } = init_node(&env, &wallet, path.clone())?;
    ...
    let hashes = env.reorg(1)?; // 102
    let new_blockhash = hashes[0];
    _ = env.mine_blocks(20, Some(miner))?; // 104
    wait_for_height(&env, 122).await?;

    // restart the node when unaware of reorg
    let LightClient {
        sender,
        mut receiver,
        node,
